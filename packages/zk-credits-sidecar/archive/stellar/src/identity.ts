...
async function systemCredentialStore(): Promise<CredentialStore> {
  const module = await import('keytar') as unknown as CredentialStore;
  return module;
}
...
export class IdentityStore {
  ...
  private async store(): Promise<CredentialStore> {
    return this.credentials ?? systemCredentialStore();
  }

  async hasIdentity(): Promise<boolean> {
    return await (await this.store()).getPassword(SERVICE, ACCOUNT) !== null;
  }
  ...
  async importMnemonic(mnemonic: string): Promise<Uint8Array> {
    ...
    await (await this.store()).setPassword(SERVICE, ACCOUNT, secretToHex(secretK));
    return secretK;
  }

  async loadSecretK(options: { headlessMnemonic?: string } = {}): Promise<Uint8Array> {
    const stored = await (await this.store()).getPassword(SERVICE, ACCOUNT);
