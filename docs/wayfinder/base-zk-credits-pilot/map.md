...
- [B22 — Invite-only unpaid x402-agent pilot launch](https://github.com/mangekyou-labs/haze-api/issues/26) — unblocked by B11 and B21. Locally implemented 2026-09-21: the single service class is enforced server-side (forced model, text and tools only, 256 KiB body, 16,000 UTF-8 byte input ceiling, 4,000 output tokens, $0.90/$1.80 per-million price ceilings, 120-second timeout, named rejections for fallback, routing, media, plugins, and unknown fields); the kill switch and the integer micro-USD provider-spend caps are durable Postgres state; `/ready` covers Postgres, Base root freshness and lag, verifier assets, provider configuration, and launch state; and authenticated aggregate status reports bounded counters, spend headroom, Claim counts, and Base lag only — see [B22 launch-control evidence](../../ai/testing/2026-09-18-feature-base-zk-credits.md#local-b22-launch-control-evidence-2026-09-21) and [the deployment checklist](../../ai/deployment/2026-09-18-feature-base-zk-credits.md#b22-launch-controls-before-invitations). The resumable launch system followed on 2026-09-21: `scripts/launch-pilot.sh` drives a 26-step checkpointed plan with an `unknown` state that blocks a retry of any non-idempotent operation, the activation evidence moved to counter snapshots so a warm-up cannot be counted as the activation, `activation-rehearse` proves the orchestration without consuming a slot, the publish sequence now covers all three packages in dependency order, and the staging-only cap pair lets cap exhaustion be exercised while production refuses it — see [B22 launch-automation evidence](../../ai/testing/2026-09-18-feature-base-zk-credits.md#local-b22-launch-automation-evidence-2026-09-21). Hosted checkpoint evidence on 2026-09-23: production readiness, fixed caps, and one pause/resume cycle passed; a separate staging Neon database and Render service were provisioned, and the real Postgres cap path exhausted a 25,000/25,000 micro-USD staging pair without touching production; the activation rehearsal also passed. The staged service has only non-secret configuration until explicit authorization is provided for runtime credential injection. Remaining before close: the three operator-owned activations, final verification/evidence, and (if hosted staging execution is required) that separately authorized credential handoff.
