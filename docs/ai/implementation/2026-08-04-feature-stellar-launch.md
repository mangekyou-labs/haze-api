...
- `packages/zk-credits-shared` (`@zk-credits/shared`, `file:` dep from `ts/` and `web/`) is the single source of the browser+Node crypto core:
  ...
  - **Hash correction:** the design doc says "Poseidon hash"; the actual circuit hash in this codebase is **MiMCSponge**. The circuits are compiled over BLS12-381 Fr, so `ts/merkle.ts` uses circomlibjs only for the standard round constants and performs MiMC arithmetic locally over BLS12-381 Fr; its BN254 convenience hash is not used. Poseidon would require CAP-0075 (not available) and is out of scope for this launch.
...
- **Hosted deployment (M3.1/3.2/3.3/3.4)** — the Fly trial ended on 2026-08-10. Render now hosts the gateway, fee-sponsor, and shared Postgres replacement; Vercel is connected to the Render gateway. Both Render services are live on `c02891c`, and a fresh hosted call produced `NullifierSpent` events with the corrected proof map, `u256` signals, and XDR event-topic filter. Legacy pre-fix queued calls still report `RootMismatch` and are not acceptance fixtures.
...
- Acceptance evidence: direct OpenRouter HTTP 200; a temporary real-proof gateway acceptance test passed with an actual upstream response and 403 replay rejection; public Stripe checkout returned to the dashboard and showed pending confirmation. Hosted funded-call settlement is now confirmed on Render/Soroban; slash, withdrawal, and restart evidence remain pending.

## BLS12-381 Merkle root alignment (2026-08-10)

- Hosted acceptance testing found that the gateway's `MerkleTree` used `circomlibjs.buildMimcSponge()` directly. That helper performs BN254 arithmetic, while the committed Circom artifacts and Soroban verifier use BLS12-381 Fr (`circom ... -p bls12381`). The resulting deposit roots were internally consistent but could not be reproduced by an RLN proof.
