...
- [x] Soroban proof-shape regression: a TDD test caught that `nativeToScVal()` encoded the snarkjs proof object in the wrong shape for the positional Soroban `Groth16Proof`; the converter now emits validated BLS12-381 G1/G2 byte values for `spend()` and `slash()`.
- [ ] Hosted funded call after BLS and proof-serialization fixes: pending the fresh Render deployment and one new test-mode deposit; legacy hosted deposits were created with the pre-fix root arithmetic and are intentionally not used as acceptance fixtures.
