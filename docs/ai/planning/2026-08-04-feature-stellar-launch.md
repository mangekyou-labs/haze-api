...
Tracking: `M1 done` -> `M2 CODE-COMPLETE (live testnet spike pending)` -> `M3 todo (3.5 CI implemented 2026-08-05)` -> `M4 todo`.
...
3. **M3.5 CI — IMPLEMENTED 2026-08-05** (workflows + web test baseline + circuit-artifact tracking); the first push to `feature-stellar-launch` triggers the CI run that validates it end-to-end.
...
- [ ] **M3 — Hosted deployment:** Soroban testnet contract, Fly.io gateway, Vercel web, fee-sponsor service, CI pipeline. — Prep: `services/fee-sponsor` package is code-complete and boots via tsx (offline); 3.1 remains a contract confirm; **3.5 (CI) implemented 2026-08-05** (workflows + web test baseline; run pending first push).
...
- [x] **3.5 Set up CI (GitHub Actions).** — IMPLEMENTED (2026-08-05); CI run pending first push
  - Outcome: CI pipeline runs gateway (`npm run test -- --coverage`), web (`npm run test` + Playwright), contract (`cargo test`), and circuit (`node scripts/test.js`) tests on push; runs an E2E smoke test on deploy; emits coverage + testnet tx hashes as redacted artifacts.
  - Depends on: 1.1, 2.4 (tests must exist).
  - Validation: CI green on push to `feature-stellar-launch`. (Covers testing: test reporting & coverage.) — **Pending first push** (worktree not yet pushed); every job's underlying command is locally verified (gateway 129/139, shared 12/12, web typecheck + 4/4 unit + 2/2 Playwright, circuits all-pass; contract `cargo test` is standard and CI pins Rust 1.94.0 — local Cargo 1.79 is too old for soroban-sdk 26).
  - Related testing scenarios: test reporting & coverage.
  - Status: `.github/workflows/ci.yml` (6-job matrix) + `.github/workflows/deploy-smoke.yml` (post-deploy health template) created; both YAML-parse clean. **Web test baseline added** (`web/src/lib/crypto.test.ts` vitest 4/4; `web/e2e/smoke.spec.ts` Playwright 2/2). **Discovered gap fixed:** circuit artifacts (`.wasm`/`*_final.zkey`) were gitignored+untracked despite being load-bearing for the CI circuits job and the browser proof path — un-ignored + committed the release artifacts. `npm ci` now works in every package (stale-`@emnapi` lockfile issue resolved by the M2 lockfile sync).
