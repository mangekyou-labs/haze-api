...
describe('requestWithdrawal (gateway co-signer + fee-relay handoff)', () => {
  ...
  it('builds the depositor-signed envelope and relays it (fee-bumps hash returned)', async () => {
    const result = await requestWithdrawal(deps, '123', 'G-RECIPIENT');

    expect(buildEnvelope).toHaveBeenCalledWith('GATEWAY-SECRET', '123', 'G-RECIPIENT');
  ...
  it('adds a fee bump only once (relay reports duplicate on retry)', async () => {
    ...
    const first = await requestWithdrawal(deps, '123', 'G-RECIPIENT');
    const second = await requestWithdrawal(deps, '123', 'G-RECIPIENT');
  ...
  it('rejects missing commitment or recipient', async () => {
    await expect(requestWithdrawal(deps, '', 'G-R')).rejects.toMatchObject({ status: 400 });
    await expect(requestWithdrawal(deps, '123', '')).rejects.toMatchObject({ status: 400 });
  });

  it('surfaces 502 when the inner tx cannot be built', async () => {
    buildEnvelope.mockRejectedValueOnce(new Error('account not found'));
    await expect(requestWithdrawal(deps, '123', 'G-R')).rejects.toMatchObject({ status: 502 });
  });

  it('surfaces 503 when the fee relay rejects', async () => {
    relayEnvelope.mockRejectedValueOnce(new Error('method not sponsored'));
    await expect(requestWithdrawal(deps, '123', 'G-R')).rejects.toMatchObject({ status: 503 });
  });
});