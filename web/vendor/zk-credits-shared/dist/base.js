...
function toField(value, label) {
    ...
    if (result < 0n || result >= BN254_FIELD_ORDER)
        throw new Error(`Invalid ${label}`);
    return result;
}
...
export function generateSecret() {
    ...
    if (bytesToBigInt(secret) >= BN254_FIELD_ORDER)
        return generateSecret();
    return secret;
}
export function secretToField(secret) {
    if (secret.length !== 32)
        throw new Error('Secret must be exactly 32 bytes');
    return toField(bytesToBigInt(secret), 'secret').toString();
}
export function secretFromBase64Url(value) {
...
export function secretToBase64Url(secret) {
    if (secret.length !== 32)
        throw new Error('Secret must be exactly 32 bytes');
    return toBase64Url(secret);
}
export async function computeCommitment(secret) {
    return poseidonHash([secretToField(secret)]);
}
export async function computeCreditLeaf(commitment, tierId, expiry) {
    if (!Number.isSafeInteger(tierId) || tierId !== FUNDED_TIER_ID)
...
export async function computeSlotBlinding(secret, slot, deploymentDomain) {
    if (!Number.isSafeInteger(slot) || slot < 0 || slot >= FUNDED_SLOT_ALLOWANCE)
        throw new Error('Invalid slot');
    return poseidonHash([secretToField(secret), slot, toField(deploymentDomain, 'deployment domain')]);
}
export async function computeNullifier(slotBlinding) {
    const blinding = toField(slotBlinding, 'slot blinding');
    if (blinding === 0n)
        throw new Error('Slot blinding must not be zero');
    return poseidonHash([blinding]);
}
export async function computeShare(secret, signal, slotBlinding) {
    const signalField = toField(signal, 'signal');
    if (signalField === 0n)
        throw new Error('Request signal must not be zero');
...
export function recoverSlotBlinding(share1, share2, signal1, signal2) {
    const x1 = toField(signal1, 'signal');
    const x2 = toField(signal2, 'signal');
    if (x1 === x2)
        throw new Error('Conflicting transcripts require different request signals');
    const s1 = toField(share1, 'share');
    const s2 = toField(share2, 'share');
    return mod((s1 - s2) * modInverse(x1 - x2)).toString();
}
export function recoverSecret(share, slotBlinding, signal) {
...
function canonicalValue(value, path = '$') {
    ...
    if (typeof value === 'object') {
        const record = value;
        return Object.fromEntries(Object.keys(record).sort().map((key) => [key, canonicalValue(record[key], `${path}.${key}`)]));
    }
    throw new Error(`Unsupported value at ${path}`);
}
export function canonicalizeBaseJson(value) {
...
export function canonicalizeRfc8785(value, path = '$') {
    if (value === null)
        return 'null';
    if (value === true)
        return 'true';
...
function requireAbsoluteHttpUrl(url) {
    let parsed;
    try {
        parsed = new URL(url);
    }
...
export async function deriveRequestSignal(input) {
    if (!/^[A-Za-z0-9_-]{16,128}$/u.test(input.nonce))
        throw new Error('Invalid request nonce');
    if (!input.responseKey)
        throw new Error('Response encryption key is required');
    if (!(input.body instanceof Uint8Array))
        throw new Error('Request body must be the exact bytes sent');
...
function validateCredentialShape(credential) {
    ...
    if (credential.tierId !== FUNDED_TIER_ID)
        throw new Error('Invalid tier id');
...
export async function createCredential(secret, tierId, expiry, deploymentDomain) {
    const credential = {
        version: CREDENTIAL_VERSION,
        secret: secretToBase64Url(secret),
...
async function deriveExportKey(password, salt, usage) {
    const keyMaterial = await crypto.subtle.importKey('raw', textEncoder.encode(password), 'PBKDF2', false, ['deriveKey']);
    return crypto.subtle.deriveKey({ name: 'PBKDF2', salt: asArrayBuffer(salt), iterations: 310_000, hash: 'SHA-256' }, keyMaterial, { name: 'AES-GCM', length: 256 }, false, [usage]);
}
async function encryptExportPayload(payload, password, version) {
...
async function decryptExportPayload(exported, password, expectedVersion) {
    requirePassword(password);
...
export async function encryptCredentialExport(credential, password) {
    const encrypted = await encryptExportPayload(credential, password, CREDENTIAL_VERSION);
    return encrypted;
}
export async function decryptCredentialExport(exported, password) {
    const parsed = await decryptExportPayload(exported, password, CREDENTIAL_VERSION);
    const credential = validateCredentialShape(parsed);
    const secret = secretFromBase64Url(credential.secret);
    if (await computeCommitment(secret) !== credential.commitment)
        throw new Error('Credential commitment mismatch');
    return credential;
}
export const CREDENTIAL_EXPORT_FORMAT = 'zk-credits-credential';
export const RECOVERY_CAPSULE_VERSION = 2;
function validateActivationMetadata(activation) {
    if (activation.tierId !== FUNDED_TIER_ID)
        throw new Error('Invalid tier id');
    if (typeof activation.commitment !== 'string'
...
export async function verifyActivatedCredential(file, password) {
    ...
    if (commitment !== activation.commitment)
        throw new Error('Credential commitment mismatch');
    return {
        version: CREDENTIAL_VERSION,
        secret: secretToBase64Url(secret),
...
export async function decryptAnyCredentialExport(file, password) {
 ...
 */
export async function deriveSparseCreditWitness(leaves, leafIndex) {
    if (!Number.isInteger(leafIndex) || leafIndex < 0 || leafIndex >= BASE_MEMBERSHIP_TREE_CAPACITY) {
        throw new Error('Invalid leaf index');
    }
    const nodes = new Map();
    for (const [index, leaf] of leaves) {
        if (!Number.isInteger(index) || index < 0 || index >= BASE_MEMBERSHIP_TREE_CAPACITY)
            throw new Error('Invalid sparse leaf index');
        nodes.set(index, toField(leaf, `leaf ${index}`).toString());
    }
    if (!nodes.has(leafIndex))
        throw new Error('Sparse leaves do not contain the requested leaf');
    const zeroHashes = ['0'];
    ...
    for (let level = 0; level < BASE_MEMBERSHIP_TREE_DEPTH; level += 1) {
        ...
        for (const index of currentNodes.keys())
